...
router.post("/asignar", async (req, res) => {
  ...
  if (!idUsuario) {
    return res
      .status(400)
      .json({
        error: "Se requiere el idUsuario al que se quiere asignar como admin.",
      });
  }
  try {
    ...
    res
      .status(201)
      .json({ mensaje: "Administrador asignado correctamente", admin });
  } catch (error) {
...
router.post("/quitar", async (req, res) => {
  ...
  if (!idUsuario) {
    return res
      .status(400)
      .json({
        error: "Se requiere el idUsuario al que se quiere quitar el rol.",
      });
  }
  try {
